...
//! # `fork()` Standalone Regression Tests
//!
//! Exercises the POSIX `fork()` library call end-to-end in standalone mode and verifies that:
//!
...
fn run_child(parent_pid: ProcessIdentifier) -> Result<(), Error> {
    ...
    let ppid: pid_t = bindings::getppid::getppid();
...
fn test_fork_cow_and_lineage() -> Result<(), Error> {
    ...
    // Child: `fork()` returns 0. The child performs its checks and terminates without returning to
    // the shared test flow, so that it never emits the success marker.
    if ret == 0 {
        let parent: ProcessIdentifier =
            match ProcessIdentifier::try_from(PARENT_PID_RAW.load(ORDER)) {
                Ok(pid) => pid,
                // SAFETY: the freshly forked child is at a safe point to terminate; it holds no locks
                // or resources that require explicit cleanup.
                Err(_) => unsafe { bindings::_exit::_exit(CHILD_EXIT_FAIL) },
            };
        let status: pid_t = match run_child(parent) {
            Ok(()) => CHILD_EXIT_OK,
            Err(_) => CHILD_EXIT_FAIL,
        };
        // SAFETY: see above; the child terminates here and never returns.
        unsafe { bindings::_exit::_exit(status) }
    }

    // Parent: a negative return indicates that `fork()` failed.
    assert!(ret > 0, "fork() failed in parent (ret={})", ret);

    let child_pid: ProcessIdentifier = ProcessIdentifier::from(ret);
    ...
    let child_ppid_raw: pid_t = i32::from_le_bytes([
        reply.payload[1],
        reply.payload[2],
        reply.payload[3],
        reply.payload[4],
...
pub fn run() -> Result<(), Error> {
    ::syslog::info!("fork-rust: starting fork() regression tests");
    test_fork_cow_and_lineage()?;
    ::syslog::info!("fork-rust: PASS - fork_cow_and_lineage");
    Ok(())
}
