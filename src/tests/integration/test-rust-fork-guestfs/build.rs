...
/// `src/tests/integration/test-fork-guestfs/src/tests/fork_fds.rs`.
...
fn main() {
    ...
    std::fs::copy(&img_path, bin_dir.join("test-rust-fork-guestfs.img"))
        .expect("failed to copy test-rust-fork-guestfs.img to bin directory");
