...
/// Exit code returned by the parent process once every `fork()` check passes.
///
...
/// the convention used by the in-kernel test program.
...
pub fn main() -> Result<(), Error> {
    ...
    Err(Error::new(
        ...
        "fork() regression suite completed successfully",
    ))
}
