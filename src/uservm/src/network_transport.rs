...
///
/// Both implementations return the real host-socket outcome of `send`/`sendto` synchronously. In
/// the remote case the calling worker blocks until the decoupled `networkd` replies, so a socket
/// write is not acknowledged to the guest before `networkd` has completed it.
///
...
/// # Write semantics
///
/// Writes (`send`/`sendto`) are **synchronous**: each call forwards to the in-process daemon and
/// returns the real host-socket outcome to the guest. [`RemoteNetwork`] preserves the same contract
/// over a socket by waiting for the decoupled `networkd` response.
///
...
impl RemoteNetwork {
    ...
    fn round_trip(&self, op: NetworkOp) -> Option<NetworkResult> {
        ...
        let frame: Vec<u8> = NetworkRequest { op }.to_frame();
