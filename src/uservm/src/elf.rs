...
///
/// Computes the memory footprint of a 32-bit ELF file.
...
fn memory_footprint_64(source: &[u8]) -> Result<MemoryFootprint> {
    let source_len: usize = source.len();
...
pub fn memory_footprint(source: &[u8]) -> Result<MemoryFootprint> {
    ...
    match source[4] {
        ...
        ELFCLASS64 => memory_footprint_64(source),
        _ => {
...
///
/// Loads a 32-bit ELF file into memory.
...
/// # Safety
///
/// - `destination` must point to a writable memory region of at least `max_offset` bytes.
/// - `source` must point to a valid ELF binary whose program headers stay within the
///   allocation backing `source`.
#[allow(unsafe_op_in_unsafe_fn)]
pub unsafe fn load(
    destination: *mut std::ffi::c_void,
    source: *const u8,
    ...
    match elf_class {
        ...
        ELFCLASS64 => load_64(destination, source, max_offset),
        _ => {
...
mod tests {
    ...
    /// Tests that memory_footprint rejects an unsupported ELF class.
    #[test]
    fn memory_footprint_rejects_invalid_class() {
        let header_size: usize = mem::size_of::<Elf32Fhdr>();

        let mut header: Elf32Fhdr = make_header(1);
        header.e_ident[4] = ELFCLASSNONE; // Invalid class.
