...
impl WhpPartition {
    ...
    pub fn new() -> Result<Self> {
        ...
        unsafe {
            ...
            if freq_mhz > 0 {
                ...
                let entry_size: u32 =
                    u32::try_from(std::mem::size_of::<WHV_X64_CPUID_RESULT>())
                ...
                .map_err(|e| {
                    let reason: String =
                        format!("failed to set CPUID result list (error={e:?})");
                    error!("WhpPartition::new(): {reason}");
                    anyhow::anyhow!(reason)
                })?;
                info!(
                    "overriding CPUID leaf 0x16: base_freq={}MHz (from {}Hz)",
                    freq_mhz, freq_hz
                );
            } else {
                warn!(
                    "could not query processor clock frequency; \
                     guest will use PIT-based calibration"
                );
            }
        }
    ...
    unsafe fn query_processor_clock_frequency() -> u64 {
