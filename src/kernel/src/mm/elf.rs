...
// ELF identification byte indices.
...
pub fn detect_elf_class(addr: usize) -> Result<ElfClass, Error> {
    ...
    match ident[EI_CLASS] {
        ELFCLASS32 => Ok(ElfClass::Elf32(Elf32Fhdr::from_address(addr))),
        ELFCLASS64 => Ok(ElfClass::Elf64(Elf64Fhdr::from_address(addr))),
...
/// - `dry_run`: If `true`, only validates the binary without allocating pages.
///
/// # Returns
///
/// Upon successful completion, the entry point and the address past the last loaded segment are
/// returned. If an error occurs, an error code is returned.
///
fn do_elf64_load(
    mm: &mut VirtMemoryManager,
    vmem: &mut Vmem,
