...
impl Inner {
    ...
    fn alloc(&mut self) -> Result<FrameAddress, Error> {
        ...
        proof! {
            ...
            lemma_index_mul_no_overflow(
                self.base@, index as int, self.bitmap@.num_bits, spec_page_size(),
            );
        ...
        proof! {
            lemma_addr_page_aligned(self.base@, index as int, spec_page_size());
        }
        let frame = FrameAddress::from_raw_value(addr)?;
        proof! {
            assert(frame@ == addr as int);
            lemma_page_index_eq(index as int, spec_page_size());
            let page_index = (frame@ - old(self)@.start) / spec_page_size();
    ...
    fn alloc_range(&mut self, count: usize, addrs: &mut Vec<FrameAddress>) -> Result<(), Error> {
        ...
        proof! {
            assert(base_addr as int == self.base@ + (index as int) * spec_page_size());
            self.lemma_internal_inv_intro();
        }
        #[verus_spec(
            invariant
        ...
        for i in 0..count
        {
            proof! {
                lemma_index_mul_no_overflow(
                    self.base@, (index as int) + (i as int), self.bitmap@.num_bits, spec_page_size(),
                );
                lemma_offset_sum(self.base@, index as int, i as int, spec_page_size());
            ...
            proof! {
                lemma_addr_page_aligned(self.base@, (index as int) + (i as int), spec_page_size());
            }
    ...
    fn free(&mut self, addr: FrameAddress) -> Result<(), Error> {
        ...
        if addr.into_raw_value() < pa_into_raw(self.base) {
            ...
            error!("{reason}");
            proof! {
                lemma_negative_div(addr@ - old(self)@.start, spec_page_size());
            }
