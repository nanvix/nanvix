...
/// # Description
///
/// A type that represents a page table (leaf level of 4-level paging).
/// Each entry is 8 bytes (u64) and there are 512 entries per table.
///
pub struct PageTable<T: DerefMut<Target = [u64]>> {
...
impl<T: DerefMut<Target = [u64]>> PageTable<T> {
    ...
    pub fn ctrl(
        ...
        let mut pte: PageTableEntry = match self.read_pte(page_address) {
            ...
            None => {
                let reason: &str = "failed to read page table entry";
                error!(
                    "change_access_permissions(): {} (page_address={:?})",
                    reason, page_address
                );
                return Err(Error::new(ErrorCode::TryAgain, reason));
            },
    ...
    fn read_pte(&self, vaddr: PageAddress) -> Option<PageTableEntry> {
        ...
        PageTableEntry::from_raw_value(self.entries[pte_idx])
    }
