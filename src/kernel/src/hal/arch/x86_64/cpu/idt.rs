...
impl Idt {
    ...
    /// # Safety
    ///
    /// The caller must ensure `idt_storage` points to at least [`IDT_LEN`] entries and
    /// `idtr_storage` to a valid [`Idtr`], both outliving all IDT usage, and that this is called
    /// at most once.
    ///
