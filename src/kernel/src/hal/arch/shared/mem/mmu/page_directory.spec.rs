...
/// Fields must be private to prevent mutation outside of TCB.
#[cfg(any(verus_keep_ghost, verus_keep_ghost_body))]
pub tracked struct NanvixPdeToken {
    ptr: *mut PteWord,
...
impl NanvixPdeToken {
    ...
    pub open spec fn admits(&self, value: PteWord) -> bool {
        self.is_init() && compatible_pde(self.expected().unwrap(), value)
    }

    /// Returns whether this token is well formed.
    pub closed spec fn wf(&self) -> bool {
        self.is_uninit() || valid_standard_pde(self.expected().unwrap())
    }
}
