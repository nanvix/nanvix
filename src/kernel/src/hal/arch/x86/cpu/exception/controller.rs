...
pub unsafe extern "C" fn do_exception(
    ...
    // SAFETY: the pointer refers to the saved register block on the kernel stack. Casting to &mut
    // is safe so that signal delivery can modify the context before returning to user space.
