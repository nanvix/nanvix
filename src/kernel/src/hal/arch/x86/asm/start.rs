...
//
// On WHP + microvm the host has already zeroed guest memory (VirtualAlloc + ELF loader
// write_bytes), so every BSS page is guaranteed to contain zeros before the vCPU starts.
// Skipping the guest-side `rep stosb` avoids touching every BSS page from inside the VM,
// which would otherwise trigger expensive EPT-violation faults (~70 µs each on WHP) for
// pages the kernel never actually reads during boot. In all other configurations we must
// zero .bss here to ensure deterministic initialization of static data.
