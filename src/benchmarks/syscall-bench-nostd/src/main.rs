...
//! shared-memory ring buffer path, including payload-carrying `pwrite()` and `pread()`
//! sweeps that cross the 4 KiB page boundary.
...
/// Payload sizes for the `pwrite()` / `pread()` sweeps.
#[cfg(feature = "payload-sweep")]
const PAYLOAD_SIZES: &[usize] = &[32, 64, 128, 256, 512, 1024, 1536, 2048, 4096, 8192, 16384, 32768];
...
fn run_payload_sweep() -> Result<(), Error> {
    ...
    let max_size: usize = max_payload_size();

    prepare_payload_file(fd, max_size, 0xa5)?;
