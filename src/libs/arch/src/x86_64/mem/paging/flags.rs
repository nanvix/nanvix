...
/// # Description
///
/// A type that represents the no-execute flag of a page table entry.
/// This flag is available on x86_64 (bit 63) and enables the NX/XD feature.
///
#[derive(Debug)]
pub enum NoExecuteFlag {
    /// Execution is allowed.
    Execute = 0,
    /// Execution is disabled.
    NoExecute = 1, // Bit 63, handled specially in into_raw_value.
}

///
/// # Description
///
/// A type that represents the global flag of a page table entry.
/// Global pages are not flushed from the TLB on CR3 writes.
///
#[derive(Debug)]
pub enum GlobalFlag {
    /// The page is not global.
    NotGlobal = 0,
    /// The page is global.
    Global = (1 << Self::SHIFT),
}
...
impl NoExecuteFlag {
    /// Bit position of the NX flag (bit 63).
    const SHIFT: u64 = 63;
    /// Bit mask of the NX flag.
    const MASK: u64 = (1u64 << Self::SHIFT);

    pub fn from_raw_value(value: u64) -> Self {
        match value & Self::MASK {
            0 => NoExecuteFlag::Execute,
            _ => NoExecuteFlag::NoExecute,
        }
    }

    pub fn into_raw_value(self) -> u64 {
        match self {
            NoExecuteFlag::Execute => 0,
            NoExecuteFlag::NoExecute => Self::MASK,
        }
    }
}

impl GlobalFlag {
    /// Bit shift of the global flag in the page table entry.
    const SHIFT: u64 = 8;
    /// Bit mask of the global flag in the page table entry.
    const MASK: u64 = (1 << Self::SHIFT);

    pub fn from_raw_value(value: u64) -> Self {
        match value & Self::MASK {
            0 => GlobalFlag::NotGlobal,
            _ => GlobalFlag::Global,
        }
    }
