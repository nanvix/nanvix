...
impl XapicIcrLo {
    /// Bit shift for vector field.
    const VECTOR_SHIFT: u32 = 0;
    /// Bit length for vector field.
    const VECTOR_BITLEN: u32 = 8;
    /// Bit mask for vector field.
    const VECTOR_MASK: u32 = (1 << Self::VECTOR_BITLEN) - 1;

    /// Bit shift for delivery mode field.
    const DELIVERY_MODE_SHIFT: u32 = 8;
    /// Bit length for delivery mode field.
    const DELIVERY_MODE_BITLEN: u32 = 3;
    /// Bit mask for delivery mode field.
    const DELIVERY_MODE_MASK: u32 = (1 << Self::DELIVERY_MODE_BITLEN) - 1;

    /// Bit shift for destination mode field.
    const DESTINATION_MODE_SHIFT: u32 = 11;
    /// Bit mask for destination mode field.
    const DESTINATION_MODE_MASK: u32 = 1;

    /// Bit shift for delivery status field.
    const DELIVERY_STATUS_SHIFT: u32 = 12;
    /// Bit mask for delivery status field.
    const DELIVERY_STATUS_MASK: u32 = 1;

    /// Bit shift for level field.
    const LEVEL_SHIFT: u32 = 14;
    /// Bit mask for level field.
    const LEVEL_MASK: u32 = 1;

    /// Bit shift for trigger mode field.
    const TRIGGER_MODE_SHIFT: u32 = 15;
    /// Bit mask for trigger mode field.
    const TRIGGER_MODE_MASK: u32 = 1;

    /// Bit shift for destination shorthand field.
    const DESTINATION_SHORTHAND_SHIFT: u32 = 18;
    /// Bit length for destination shorthand field.
    const DESTINATION_SHORTHAND_BITLEN: u32 = 2;
    /// Bit mask for destination shorthand field.
    const DESTINATION_SHORTHAND_MASK: u32 = (1 << Self::DESTINATION_SHORTHAND_BITLEN) - 1;

    /// Creates a new ICR (Low) register.
    pub fn new(
        vector: u32,
        delivery_mode: XapicIcrDeliveryMode,
        destination_mode: XapicIcrDestinationMode,
        level: XapicIcrLevel,
        trigger_mode: XapicIcrTriggerMode,
        destination_shorthand: XapicIcrDestinationShorthand,
    ) -> Self {
        Self {
            bits: ((vector & Self::VECTOR_MASK) << Self::VECTOR_SHIFT)
                | ((delivery_mode as u32 & Self::DELIVERY_MODE_MASK) << Self::DELIVERY_MODE_SHIFT)
                | ((destination_mode as u32 & Self::DESTINATION_MODE_MASK)
                    << Self::DESTINATION_MODE_SHIFT)
                | ((level as u32 & Self::LEVEL_MASK) << Self::LEVEL_SHIFT)
                | ((trigger_mode as u32 & Self::TRIGGER_MODE_MASK) << Self::TRIGGER_MODE_SHIFT)
                | ((destination_shorthand as u32 & Self::DESTINATION_SHORTHAND_MASK)
                    << Self::DESTINATION_SHORTHAND_SHIFT),
        }
    }

    /// Returns the vector field.
    pub fn vector(&self) -> u32 {
        self.bits & Self::VECTOR_MASK
    }

    /// Returns the delivery mode field.
    pub fn delivery_mode(&self) -> u32 {
        (self.bits >> Self::DELIVERY_MODE_SHIFT) & Self::DELIVERY_MODE_MASK
    }

    /// Returns the destination mode field.
    pub fn destination_mode(&self) -> bool {
        ((self.bits >> Self::DESTINATION_MODE_SHIFT) & Self::DESTINATION_MODE_MASK) != 0
    }

    /// Returns the delivery status field.
    pub fn delivery_status(&self) -> bool {
        ((self.bits >> Self::DELIVERY_STATUS_SHIFT) & Self::DELIVERY_STATUS_MASK) != 0
    }

    /// Returns the level field.
    pub fn level(&self) -> bool {
        ((self.bits >> Self::LEVEL_SHIFT) & Self::LEVEL_MASK) != 0
    }

    /// Returns the trigger mode field.
    pub fn trigger_mode(&self) -> bool {
        ((self.bits >> Self::TRIGGER_MODE_SHIFT) & Self::TRIGGER_MODE_MASK) != 0
    }

    /// Returns the destination shorthand field.
    pub fn destination_shorthand(&self) -> u32 {
        (self.bits >> Self::DESTINATION_SHORTHAND_SHIFT) & Self::DESTINATION_SHORTHAND_MASK
    }

    /// Returns the raw value of the ICR (Low) register.
    pub fn to_u32(&self) -> u32 {
        self.bits
    }

    /// Creates a new ICR (Low) register from a raw value.
    pub fn from_u32(bits: u32) -> Self {
        Self { bits }
    }
}
...
/// Destination mode.
#[repr(u8)]
pub enum XapicIcrDestinationMode {
    /// Physical mode.
    Physical = 0,
    /// Logical mode.
    Logical = 1,
