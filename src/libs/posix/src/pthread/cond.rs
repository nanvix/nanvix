...
pub unsafe extern "C" fn pthread_cond_broadcast(cond: *const pthread_cond_t) -> c_int {
    ...
    match pthread::pthread_cond_broadcast(&*cond) {
        Ok(()) => 0,
...
pub unsafe extern "C" fn pthread_cond_init(
    ...
    match pthread::pthread_cond_init(&mut *cond, &attr) {
        Ok(()) => 0,
        Err(error) => error.code.get(),
    }
}
...
pub unsafe extern "C" fn pthread_cond_destroy(cond: *mut pthread_cond_t) -> c_int {
    ...
    match pthread::pthread_cond_destroy(&mut *cond) {
        Ok(()) => 0,
        Err(error) => error.code.get(),
    }
}
...
pub unsafe extern "C" fn pthread_cond_signal(cond: *const pthread_cond_t) -> c_int {
    ...
    match pthread::pthread_cond_signal(&*cond) {
        Ok(()) => 0,
        Err(error) => error.code.get(),
    }
}
...
pub unsafe extern "C" fn pthread_cond_timedwait(
    ...
    match pthread::pthread_cond_timedwait(&*cond, &*mutex, Some(timeout)) {
        Ok(()) => 0,
        Err(error) => {
            ::syslog::error!(
...
pub unsafe extern "C" fn pthread_cond_wait(
    ...
    match pthread::pthread_cond_wait(&*cond, &*mutex) {
        Ok(()) => 0,
        Err(error) => error.code.get(),
    }
}
