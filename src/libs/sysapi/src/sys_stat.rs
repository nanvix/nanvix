...
pub enum StatError {
    ...
    InvalidSize,
    /// Error code indicating failure to parse `st_dev` field.
    FailedToParseDev,
    /// Error code indicating failure to parse `st_ino` field.
    FailedToParseIno,
    /// Error code indicating failure to parse `st_mode` field.
    FailedToParseMode,
    /// Error code indicating failure to parse `st_nlink` field.
    FailedToParseNlink,
    /// Error code indicating failure to parse `st_uid` field.
    FailedToParseUid,
    /// Error code indicating failure to parse `st_gid` field.
    FailedToParseGid,
    /// Error code indicating failure to parse `st_rdev` field.
    FailedToParseRdev,
    /// Error code indicating failure to parse `st_size` field.
    FailedToParseSize,
    /// Error code indicating failure to parse `st_atim` field.
    FailedToParseAtim,
    /// Error code indicating failure to parse `st_mtim` field.
    FailedToParseMtim,
    /// Error code indicating failure to parse `st_ctim` field.
    FailedToParseCtim,
    /// Error code indicating failure to parse `st_blksize` field.
    FailedToParseBlksize,
    /// Error code indicating failure to parse `st_blocks` field.
    FailedToParseBlocks,
}
...
impl stat {
    ...
    const SIZE_OF_ST_DEV: usize = size_of::<dev_t>();
    /// Size of the file serial number field.
    const SIZE_OF_ST_INO: usize = size_of::<ino_t>();
    /// Size of the file mode field.
    const SIZE_OF_ST_MODE: usize = size_of::<mode_t>();
    /// Size of the link count field.
    const SIZE_OF_ST_NLINK: usize = size_of::<nlink_t>();
    /// Size of the user ID field.
    const SIZE_OF_ST_UID: usize = size_of::<uid_t>();
    /// Size of the group ID field.
    const SIZE_OF_ST_GID: usize = size_of::<gid_t>();
    ...
    pub fn to_bytes(&self) -> [u8; Self::SIZE] {
        ...
        // Convert block size field (always as i64 for IPC compatibility).
        // Cast is needed on x86 (c_long=i32→i64) but is a no-op on x86_64 (c_long=i64).
        #[allow(clippy::unnecessary_cast)]
        let blksize_bytes = (self.st_blksize as i64).to_ne_bytes();
        bytes[Self::OFFSET_OF_ST_BLKSIZE..Self::OFFSET_OF_ST_BLKSIZE + Self::SIZE_OF_ST_BLKSIZE]
            .copy_from_slice(&blksize_bytes);

        // Convert number of blocks allocated field (always as i64 for IPC compatibility).
        #[allow(clippy::unnecessary_cast)]
        let blocks_bytes = (self.st_blocks as i64).to_ne_bytes();
    ...
    pub fn try_from_bytes(bytes: &[u8]) -> Result<Self, StatError> {
        // Check if the array has the correct size.
        if bytes.len() != Self::SIZE {
            return Err(StatError::InvalidSize);
        }
        ...
        let st_dev: dev_t = dev_t::from_ne_bytes(
            bytes[Self::OFFSET_OF_ST_DEV..Self::OFFSET_OF_ST_DEV + Self::SIZE_OF_ST_DEV]
                .try_into()
                .map_err(|_| StatError::FailedToParseDev)?,
        );
        ...
        let st_ino: ino_t = ino_t::from_ne_bytes(
            bytes[Self::OFFSET_OF_ST_INO..Self::OFFSET_OF_ST_INO + Self::SIZE_OF_ST_INO]
                .try_into()
                .map_err(|_| StatError::FailedToParseIno)?,
        );
        ...
        let st_mode: mode_t = mode_t::from_ne_bytes(
            bytes[Self::OFFSET_OF_ST_MODE..Self::OFFSET_OF_ST_MODE + Self::SIZE_OF_ST_MODE]
                .try_into()
                .map_err(|_| StatError::FailedToParseMode)?,
        );
        ...
        let st_nlink: nlink_t = nlink_t::from_ne_bytes(
            bytes[Self::OFFSET_OF_ST_NLINK..Self::OFFSET_OF_ST_NLINK + Self::SIZE_OF_ST_NLINK]
                .try_into()
                .map_err(|_| StatError::FailedToParseNlink)?,
        );
        ...
        let st_uid: uid_t = uid_t::from_ne_bytes(
            bytes[Self::OFFSET_OF_ST_UID..Self::OFFSET_OF_ST_UID + Self::SIZE_OF_ST_UID]
                .try_into()
                .map_err(|_| StatError::FailedToParseUid)?,
        );
        ...
        let st_gid: gid_t = gid_t::from_ne_bytes(
            bytes[Self::OFFSET_OF_ST_GID..Self::OFFSET_OF_ST_GID + Self::SIZE_OF_ST_GID]
                .try_into()
                .map_err(|_| StatError::FailedToParseGid)?,
        );
        ...
        let st_rdev: dev_t = dev_t::from_ne_bytes(
            bytes[Self::OFFSET_OF_ST_RDEV..Self::OFFSET_OF_ST_RDEV + Self::SIZE_OF_ST_RDEV]
                .try_into()
                .map_err(|_| StatError::FailedToParseRdev)?,
        );
        ...
        let st_size: off_t = off_t::from_ne_bytes(
            bytes[Self::OFFSET_OF_ST_SIZE..Self::OFFSET_OF_ST_SIZE + Self::SIZE_OF_ST_SIZE]
                .try_into()
                .map_err(|_| StatError::FailedToParseSize)?,
        );
