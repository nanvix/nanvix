...
//
// This file contains the abstract pool model, its invariant and geometry helpers,
// and the numeric specification of `align_up`.
...
impl BumpView {
    ...
    /// Well-formedness of the abstract pool.
...
///
/// Uninterpreted: the dynamic `allocated` field abstracts the interior-mutable
/// `AtomicUsize` cursor, whose value is not spec-readable.
///
...
/// `FixedSizeBumpAllocator` alongside the exec method block is currently not accepted by Verus.
