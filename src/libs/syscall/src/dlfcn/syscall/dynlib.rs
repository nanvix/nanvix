...
impl DynamicLibrary {
    ...
    /// held while invoking destructors, so a destructor that tries to
    /// re-enter `libposix`'s pthread or alloc paths does not deadlock
    /// on its own per-library lock.
    ///
    /// The current `dlclose()` caller still holds
    /// `DYNAMIC_LIBRARY_REGISTRY` for the duration of the unload, AND
    /// the library being closed has already been removed from the
    /// registry before destructors run.  As a result, **no `dlfcn`
    /// API can be re-entered from a destructor today** -- not
    /// `dlopen`, not `dlclose`, not even `dlsym(self_handle, ...)`
    /// (the self lookup would either fail because the library is no
    /// longer in the registry, or block on the registry mutex).
    /// Lifting this restriction requires releasing the registry lock
    /// across destructor execution, which is tracked separately; see
    /// the loader reentrancy docs.
    ///
    /// # Safety
    ///
