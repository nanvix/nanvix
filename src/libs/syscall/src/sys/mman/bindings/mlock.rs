...
/// # Description
///
/// Locks into memory the whole pages containing any part of the address range that starts at
/// `addr` and spans `length` bytes, guaranteeing that they remain memory-resident.
///
/// Nanvix never swaps or pages out memory, so every mapped page is permanently resident. Locking a
/// resident page is therefore a no-op, and this function only validates its arguments.
///
/// # Parameters
///
/// - `addr`: Start address of the region to lock.
...
///
/// - Locking is a no-op: mapped pages are always resident.
///
#[cfg(feature = "syscall")]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mlock(addr: *const c_void, length: c_size_t) -> c_int {
    match super::validate_lock_range(addr as usize, length) {
        Ok(None) => 0,
        Ok(Some((addr, length))) => {
            let addr: VirtualAddress = VirtualAddress::from_raw_value(addr);
            match mman::mlock(addr, length) {
                Ok(()) => 0,
                Err(error) => {
                    unsafe {
