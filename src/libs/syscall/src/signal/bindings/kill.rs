...
/// # Process groups
///
/// The POSIX process-group selectors are honored: a `pid` of `0` signals the caller's process
/// group, `-1` broadcasts to every signalable process, and a `pid` less than `-1` signals the
/// process group whose identifier is `-pid`. These selectors are resolved by the process manager
/// daemon, which owns process-group state.
#[unsafe(no_mangle)]
#[trace_syscall]
