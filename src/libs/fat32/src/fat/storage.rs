...
impl RawMemoryStorage {
    ...
    /// # Returns
    ///
    /// `Ok(Self)` on success, or `Err(FsError::InvalidArgument)` if `base` is
    ...
    #[inline]
    pub unsafe fn new(base: *mut u8, size: usize) -> Result<Self, FsError> {
        if base.is_null() {
...
impl Seek for RawMemoryStorage {
    fn seek(&mut self, pos: SeekFrom) -> Result<u64, Self::Error> {
        ...
        let new_pos: usize = new_pos as usize;
