...
::static_assert::assert_eq!(
    mem::size_of::<SystemMessageHeader>() + mem::size_of::<ProcessManagementMessageHeader>()
        == REQUEST_IDENTIFIER_OFFSET
);
::static_assert::assert_eq!(
    mem::offset_of!(SystemMessage, payload) + mem::offset_of!(ProcessManagementMessage, payload)
        == REQUEST_IDENTIFIER_PREFIX_SIZE
);
...
impl ProcessManagementMessage {
    ...
    pub fn new(header: ProcessManagementMessageHeader, payload: [u8; Self::PAYLOAD_SIZE]) -> Self {
        Self {
            header,
            request_id: RequestIdentifier::NONE.raw(),
            payload,
        }
    }
...
mod tests {
    ...
    fn process_management_request_id_uses_shared_raw_offset() {
        ...
        assert_eq!(
            &bytes[REQUEST_IDENTIFIER_OFFSET..REQUEST_IDENTIFIER_OFFSET + RequestIdentifier::SIZE],
            &request_id.to_ne_bytes()
        );
