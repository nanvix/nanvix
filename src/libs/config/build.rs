...
fn main() {
    // Read the TOML file
    let kernel_config_content = fs::read_to_string("../../../config/kernel_config.toml").expect("Failed to read kernel_config.toml");
    ...
    if let Some(memory_size) = kernel_config.get("memory_size").and_then(|v| v.as_integer()) {
    ...
    if let Some(kstack_size) = kernel_config.get("kstack_size").and_then(|v| v.as_integer()) {
        constants.push_str(&format!("pub const KSTACK_SIZE: usize = {};\n", kstack_size));
    }
    if let Some(ustack_size) = kernel_config.get("ustack_size").and_then(|v| v.as_integer()) {
        constants.push_str(&format!("pub const USTACK_SIZE: usize = {};\n", ustack_size));
    }
    ...
    if let Some(scheduler_freq) = kernel_config.get("scheduler_freq").and_then(|v| v.as_integer()) {
        constants.push_str(&format!("pub const SCHEDULER_FREQ: usize = {};\n", scheduler_freq));
    }
    if let Some(max_ikc_messages) = kernel_config.get("max_ikc_messages").and_then(|v| v.as_integer()) {
        constants.push_str(&format!("pub const MAX_IKC_MESSAGES: usize = {};\n", max_ikc_messages));
    }
    if let Some(ipc_message_size) = kernel_config.get("ipc_message_size").and_then(|v| v.as_integer()) {
    ...
    println!("cargo:rerun-if-changed=kernel_config.toml");
}
