...
impl Benchmark {
    ...
    fn start_nanvixd(&self, linuxd_deployment: &LinuxdDeployment) -> Result<Child> {
        let nanvixd_program: String = format!("{}/bin/nanvixd.elf", self.workspace_root.display());
        ...
        if *linuxd_deployment == LinuxdDeployment::L2Vm {
            ...
            nanvixd_args.push(::nanvixd::args::Args::OPT_NETNS_POOL_SIZE.to_string());
            nanvixd_args.push("0".to_string());
        }

        let mut command: Command =
            if self.nanvixd_use_sudo && *linuxd_deployment == LinuxdDeployment::L2Vm {
                let mut command: Command = Command::new("sudo");
                command.arg("--").arg(&nanvixd_program);
                command
            } else {
                Command::new(&nanvixd_program)
            };
        command
            .args(&nanvixd_args)
            .stdin(if self.nanvixd_use_sudo {
                Stdio::inherit()
            } else {
                Stdio::null()
            })
        ...
        debug!(
            "Starting nanvixd with command: {} {}",
            if self.nanvixd_use_sudo && *linuxd_deployment == LinuxdDeployment::L2Vm {
                "sudo --"
            } else {
                nanvixd_program.as_str()
            },
            if self.nanvixd_use_sudo && *linuxd_deployment == LinuxdDeployment::L2Vm {
                std::iter::once(nanvixd_program.as_str())
                    .chain(nanvixd_args.iter().map(String::as_str))
                    .collect::<Vec<_>>()
                    .join(" ")
            } else {
                nanvixd_args.join(" ")
        ...
        let nanvixd_cmd: Child = command.spawn()?;
    ...
    pub async fn run_warm_start_vmm(&mut self) -> Result<()> {
        ...
        for _ in 0..self.iterations {
            ...
            let ipc_read_message: Message = match vcpu_thread_stdout_rx.recv().await {
                ...
                Some(IkcFrame::Fixed(fixed)) => {
                    let reason: String = format!(
                        "unexpected fixed-buffer transfer received while waiting for ReadRequest \
                         (buffer_id={}, len={})",
                        fixed.buffer_id(),
                        fixed.data_len()
                    );
            ...
            let pull_header: DataChunkHeader = match vcpu_thread_stdout_rx.recv().await {
                ...
                Some(IkcFrame::Fixed(fixed)) => {
                    let reason: String = format!(
                        "unexpected fixed-buffer transfer received while waiting for bulk pull \
                         request (buffer_id={}, len={})",
                        fixed.buffer_id(),
                        fixed.data_len()
                    );
                    error!("run_warm_start_vmm(): {reason}");
            ...
            let _write_request: Message = match vcpu_thread_stdout_rx.recv().await {
                ...
                Some(IkcFrame::Fixed(fixed)) => {
                    let reason: String = format!(
                        "unexpected fixed-buffer transfer received while waiting for WriteRequest \
                         (buffer_id={}, len={})",
                        fixed.buffer_id(),
                        fixed.data_len()
                    );
            ...
            let _push_data: DataChunk = match vcpu_thread_stdout_rx.recv().await {
                ...
                Some(IkcFrame::Fixed(fixed)) => {
                    let reason: String = format!(
                        "unexpected fixed-buffer transfer received while waiting for bulk push \
                         data (buffer_id={}, len={})",
                        fixed.buffer_id(),
                        fixed.data_len()
                    );
