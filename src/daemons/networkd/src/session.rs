    ...
    let sock: &mut SocketState = match session.sockets.get_mut(&host_fd) {
        ...
        None => {
            ...
            if let Some(result) = ops::abort_parked(op, ErrorCode::BadFile).response {
                push_response(out, NetworkResponse { tid, result });
            }
            return;
        },
    };
...
/// Encodes `response` into a wire frame and queues it, logging and dropping it if encoding fails.
///
/// Encoding only fails for a response whose payload exceeds the wire limits; such a response cannot
/// be delivered, so the reactor logs it and moves on rather than tearing down the session.
fn push_response(out: &mut Vec<Vec<u8>>, response: NetworkResponse) {
    match response.to_frame() {
        Ok(frame) => out.push(frame),
        Err(e) => error!("networkd reactor: failed to encode response frame: {e}"),
    }
}
...
mod tests {
    ...
    fn recv_parks_then_resumes_after_peer_send() {
        ...
        let mut events: [EpollEvent; 8] = [EpollEvent::default(); 8];
