...
Options:
  ...
  --git-commit          Auto git-commit spec/proof files after successful verification.
...
VERIFIED="$(grep -oP '\d+ verified' "$TMPFILE" | head -1 || echo "0 verified")"
ERRORS="$(grep -oP '\d+ errors' "$TMPFILE" | head -1 || echo "0 errors")"
...
elif [[ $((ASSUME_COUNT + EXTERNAL_BODY_COUNT + ADMIT_COUNT + TRUSTED_COUNT + NO_DECREASES_COUNT)) -eq 0 ]]; then
...
echo "=== Summary ==="
echo "  verification: $VERIFIED, $ERRORS (exit $VERUS_EXIT)"
echo "  cheating: assume=$ASSUME_COUNT external_body=$EXTERNAL_BODY_COUNT admit=$ADMIT_COUNT trusted=$TRUSTED_COUNT no_decreases=$NO_DECREASES_COUNT"
if [[ -n "$COV_TOTAL" ]]; then
    echo "  coverage: $COV_VERIFIED/$COV_TOTAL exec functions have contracts"
fi
if [[ "$CHEATING_FOUND" == "true" ]]; then
...
if [[ "$GIT_COMMIT" == "true" ]]; then
    ...
    if [[ $VERUS_EXIT -eq 0 && "$CHEATING_FOUND" != "true" ]]; then
        ...
        COMMIT_MSG="[verus] verify PASS (cheating detected): ${LABEL} ($VERIFIED, $ERRORS, admit=$ADMIT_COUNT)"
    else
