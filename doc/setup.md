...
> ⚠️ For this step, case you will build QEMU, remove QEMU from ubuntu.sh.
...
exec bash  # Restart shell to update path.
...
```bash
cd $WORKDIR/nanvix/src/kernel
make MICROVM_PATH=$WORKDIR/nanvix/src/microvm/bin TARGET=x86 MACHINE=microvm TIMEOUT=90 run
```
