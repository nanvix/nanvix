...
```text
+-------------------------+----------------+------------------------+
| Operation kind          | Request ID     | Operation payload      |
+-------------------------+----------------+------------------------+
```
...
Direct host-I/O and socket transfers carry the request ID through `GuestSgBulkHeader` and
`HostBulkTransferHeader`. UserVM retains metadata requests in a bounded queue and matches an
arriving bulk frame by source PID, source TID, and request ID instead of assuming frame adjacency.
Ordinary unmatched metadata expires after five seconds; counted console polls are trusted VFSD
traffic and retain reserved queue capacity.

A read exchange is ordered as follows:

1. Client sends `ReadRequest(fd, count)`.
2. Client calls `__kcall_pull_tagged()` and waits for `vfsd` to push bytes.
...
2. Client calls `__kcall_push_tagged()` and `vfsd` performs the matching tagged pull.
...
The protocol supports open, close, read, write, seek, flush, truncate, timestamp and ownership
updates, permission and access checks, hard links, mkdir, rmdir, unlink, rename, stat variants,
symlink, readlink, and directory iteration. Variable-length HostFS path requests use the same
multipart framing as guest long messages, with the operation ID at the start of the assembled body.
...
Read, write, and seek requests on one remote open-file description are serialized so asynchronous
HostFS completion cannot reorder shared-offset updates. Reads and writes use explicit positional
requests; the local shared offset advances only after successful data delivery or write completion.
A completed read whose caller has not registered its pull is retained and retried with bounded
backoff by the helper timer thread.
...
- Only `vfsd` may submit internal console, pipe, and HostFS read-retry messages.
...
- UserVM bounds unmatched bulk metadata and rejects excess requests rather than buffering
  indefinitely.
