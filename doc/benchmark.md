...
`nanvix-bench` currently supports three different benchmarks:
...
3. `echo-breakdown`: breaks down the overheads of a warm start by time-stamping messages as they flow through the system.

you may run each experiment with:

```bash
./bin/nanvix-bench.elf -benchmark [cold-start,warm-start,echo-breakdown]
```

> ℹ️ **Note:** To run `cold-start` and `warm-start`, compile Nanvix with `make all RELEASE=yes LOG_LEVEL=panic`.
