...
Processes are abstraction of running programs and play a central role in Nanvix. Programs may create new processes by calling \texttt{fork()}, which creates an exact copy of the current running process. The primal process is called the parent and the new process is called child, and they have the same core\footnote{code, stack and data segments}, opened files, pending signals and execution flow, differing only differ in their ID number. Processes may query about their ID by calling \texttt{getpid()} and may change replace their core by another executable file through the \texttt{execve()} system call. When the process is done, it invokes \texttt{exit()} to relinquish all resources that it was using.
...
\begin{tabular}{l l l}
	...
	\midrule
	\multirow{7}{*}{Process Management}  & \texttt{execve}   & Executes a program                                  \\%
