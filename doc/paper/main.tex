...
% Process states.
A process is created whenever a user launches a program, or the system itself spawns a new daemon. After that, the new process goes through a complex lifetime before it terminates (Figure (??)). Initially this new processes is ready to execute (state ??), and it is eventually selected to run by the scheduler, moving to state (??). At this moment, the process resumes back its execution in kernel land (state ??) and realizes that it should jump to userland, going to state ??. There, the process performs some computation, and it may either gets preempted if it runs out of quantum time, moving to state ??, or gets blocked waiting for a resource, going to state ??. Either way, the process later resumes its work, and loops back on these states. However, when the process finally gets his job done, or it somehow crashes, it terminates and becomes a zombie process (state ??). A zombie process awaits to gets all resources assigned to it to be taken away by the kernel, and then it is turned to a dead process (state ??).

\subsection{Process Scheduling and Switching}

% Process scheduling.
%Several processes may be active in the system, but only one can be running at a time\footnote{Nanvix does not support multiprocessor systems, thus process are not indeed running in parallel.}. The scheduler chooses what process to run next whenever the running process either gets preempted, blocks awating for a resource, or terminates. When the new processes is selected, it is assigned a new quantum of time. When the process is running in user mode, this time is preempted and after that it gets preempted.
