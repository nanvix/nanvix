...
% Provided interface.
The memory management subsystem maintains memory regions in a global table and exports a well defined interface for dealing with them. Some routines provided by this interface are listed in Table (??) and briefly detailed next. The \texttt{allocreg()} allocates a new memory region, by searching an empty slot in the memory regions table. Conversely, \texttt{freereg()} frees a region table by erasing the corresponding entry from the table. After a region has been created, \texttt{loadreg()} may be invoked to actually load it with some data, most likely from a binary file. 
