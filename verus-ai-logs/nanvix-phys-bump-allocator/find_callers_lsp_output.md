...
### `new` (impl `FixedSizeBumpAllocator<N, A, S>`) [pub] — **0 external callers**
```
...
### `alloc_as` (impl `FixedSizeBumpAllocator<N, A, S>`) [pub] — **0 external callers**
```
...
### `align_up` [pub] — **0 external callers**
```
...
### `FixedSizeBumpAllocator` [pub] — 0 external reference(s)
